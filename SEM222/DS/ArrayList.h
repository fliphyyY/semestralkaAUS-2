...
{	/*template<typename T>
	...
	void ArrayList<T>::insert(const T& data, const int index)
	{
		...
		/*DSRoutines::rangeCheck(index, size_ + 1, true, &std::string("Index out of range in ArrayList!"));
		ensureCapacity();
		if (index == 0) {
		add(data);
		}
		else {
		array_->copy(*array_, index, *array_, index + 1, ((int)size_ - index));
		(*array_)[index] = data;
		size_++;
	...
	template<typename T>
	inline T & ArrayList<T>::getDataOf(const int index)
	{
		DSRoutines::rangeCheck(index, size_, true);
		return (*array_)[index];
	}
